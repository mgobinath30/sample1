...
;(function(window) {
	...
	function toggleSlide(dir, delay) {
		var slide = DOM.slides[currentRoom],
			...
			// image = slide.querySelector('.slide__img');

		delay = delay !== undefined ? delay : 0;

		// anime.remove([name, title, date]);
		anime.remove([title, date]);

		// anime.remove([title, date, image]);
		var animeOpts = {
			// targets: [name, title, date],
			// targets: [title, date, image],
			targets: [title, date],
	...
	function showMenu() {
		...
		document.querySelector('.overlay').setAttribute('style','pointer-events:auto')

		document.querySelector('.overlay').classList.add('pointerEvents');
	...
	function closeMenu() {
		// Button becomes menu.
		DOM.menuCtrl.classList.remove('btn--active pointerEvents');
	    document.querySelector('.overlay').remove('pointerEvents');
